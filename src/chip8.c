...
void set_register(chip8_t *chip8, char index, int value) {
    chip8->V[index] = value;
}

// Should add to the value at the specified register index
void add_to_register(chip8_t *chip8, char index, int value) {
    chip8->V[index] += value;
}
